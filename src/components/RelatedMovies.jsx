...
export function RelatedMovies({ item }) {
    ...
    return <div className="rounded-md max-w-[90%] h-[12rem] sm:h-[23rem] ">
        <div className={`sm:bg-cover bg-contain bg-no-repeat sm:bg-center sm:h-[15vh] 400:h-[5.75rem] 375:h-[5rem] bg-top relative rounded-t-md group cursor-pointer`} style={{ backgroundImage: `url('${image720Url}${item?.backdrop_path || item?.poster_path || "/default-placeholder.png"}')` }}>
